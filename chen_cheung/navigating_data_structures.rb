...
while i < blog_posts.size do
  post = i
  html << title_to_html(blog_posts[i][:title])
  html << author_to_html(blog_posts[i][:author][:first_name], blog_posts[i][:author][:last_name], blog_posts[i][:author][:twitter_handle])
  html << body_to_html(blog_posts[i][:body])
  html << tags_to_html(blog_posts[i][:tags])
  i = i + 1
end
