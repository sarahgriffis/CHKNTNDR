...
stop = blog_posts.size-1
0.upto stop do |n|
  post = blog_posts[n]
  currentTitle = title_to_html post[:title]
  html << currentTitle

  currentAuthor = author_to_html post[:author][:first_name], post[:author][:last_name], 
    post[:author][:twitter_handle]
  html << currentAuthor

  currentBody = body_to_html post[:body]
  html << currentBody

  currentTags = tags_to_html post[:tags]
  html << currentTags
end
